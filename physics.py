...
class APIMassBalanceAnalyzer:
    ...
    def calculate_interval(self, hole_in, length_ft, washout, target_lgs_frac, lithology, active_equipment: list):
        # 1. Calculate Total Generated Cuttings Volume
        v_c_total = 0.000971 * (hole_in**2) * length_ft * washout
        v_h = v_c_total  

        # 2. LITHOLOGY PENALTY (Applying your idea correctly)
        # Reactive clay creates more ultrafine particles initially compared to sandstone
        ...
        # EXACT volume of solids failing to be removed
        v_c_retained = np.sum(current_psd) 

        # 4. Economics & Dilution
        ...
        # Calculate the actual LGS % that the physics engine will use
        ...
        return {
            ...
            "v_liquid_waste": v_lw,
            "system_efficiency_pct": (total_solids_discarded / v_c_total) * 100 if v_c_total > 0 else 0,
            "cost_mud": cost_mud,
            "cost_disposal": cost_disposal,
            "total_equip_rent": total_daily_rent,
            "actual_lgs_pct": actual_lgs_pct # <-- This is what feeds into AdvancedDrillingPhysics!
        }

class AdvancedDrillingPhysics:
    def __init__(self, t600, t300, t200, t100, t6, t3):
        # Strict mathematical protection, NO try-except band-aids.
    ...
    def calculate_rheology(self, lgs_pct, temp_f):
        # Arrhenius thermodynamic model for polymer degradation
